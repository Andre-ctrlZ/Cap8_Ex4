...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y)
{
  ...
  // Para números de 0 a 9
  else if (c >= '0' && c <= '9')
  {
    index = (c - '0' + 1) * 8; // Adiciona o deslocamento necessário para os números
  } 
  // Para letras minúsculas
  else if (c >= 'a' && c <= 'z') 
  {
    index = (c - 'a' + 37) * 8; // Letras minúsculas começam após as maiúsculas e os números
  }

  // Desenho do caractere
  for (uint8_t i = 0; i < 8; ++i)
  {
    uint8_t line = font[index + i]; // Pega a linha correspondente da fonte
